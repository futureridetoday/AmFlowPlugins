...
Cria um novo recurso AmFlow. Tipos suportados: `skill`, `agent`, `module` — redução temporária, até cada tipo ter seu fluxo dedicado (`hook`, `command` e `plugin` saem por ora; `workflow` sai em definitivo — autoria de workflow migra para módulo). Para `skill`, o Creator escolhe o método — survey guiado, importar um recurso existente, ou partir do template puro; `agent` e `module` partem do template com o nome.
